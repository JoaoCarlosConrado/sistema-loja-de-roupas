...
public class CategoryDao {
    ...
    public static List<Category> findAll() {
        ...
        return repCategory.find(fo).toList();
    }
    
    public static Category findById(NitriteId id) {
        return repCategory.getById(id);
    }
    
    public static Category findByName(String name) {
        return repCategory.find(ObjectFilters.eq("category", name)).firstOrDefault();
    }
    
    public static List<Category> search(String arg0) {
        return repCategory.find(ObjectFilters.regex("category", arg0),FindOptions.sort("category", SortOrder.Ascending)).toList();
    }
